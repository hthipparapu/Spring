...
```{r}
## Load Patient
patient.dt patient.dt <- fread("Patient.csv")
...
merged.dt[, IsDeceased := !is.na(DateOfDeath)]
...
disease.dt <- merge(outpatient.visit.dt, diseasemap.dt, by.x = c("ICD_1", "ICD_2", "ICD_3"), by.y = "ICDCode", all.x = TRUE)
