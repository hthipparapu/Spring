...
# Calculate mortality by gender
female.mort <- merged.dt[Gender == "Female", mean(IsDeceased, na.rm = TRUE)]
male.mort <- merged.dt[Gender == "Male", mean(IsDeceased, na.rm = TRUE)]
```
...
patients_with_visits <- unique(outpatient.visit.dt[, PatientID])
all_codes <- rbind(
...
all_codes <- unique(all_codes[!is.na(ICD)])
patient_conditions <- merge(all_codes, diseasemap.dt, by.x = "ICD", by.y = "ICD10", allow.cartesian = TRUE)
patient_conditions <- unique(patient_conditions[, .(PatientID, Condition)])
patient_disease_gender <- merge(patient_conditions, patient.dt[, .(PatientID, Gender)], by = "PatientID")

# Calculate rates
total_patients <- patient.dt[PatientID %in% patients_with_visits, .N, by = Gender]
total_all <- sum(total_patients$N)
summaries.dt <- patient_disease_gender[, .(
  Men = sum(Gender == "male") / total_patients[Gender == "male", N] * 100,
  Women = sum(Gender == "female") / total_patients[Gender == "female", N] * 100,
  All = .N / total_all * 100
), by = Condition]

summaries.dt[, c("Men", "Women", "All") := lapply(.SD, round, 1), .SDcols = c("Men", "Women", "All")]
setorder(summaries.dt, Condition)
print(summaries.dt)
...
```{r}
# Function to get the interval (start and end dates) for a given year
...
# Function to calculate the mortality rate for a given year
get_mortality_rate <- function(yr) {
  # Get the start and end dates for the year
  interval <- get_interval(yr)
  
  # Get deaths that occurred during the specified year
  deaths.dt <- mortality.dt[DateOfDeath >= interval$start.date & DateOfDeath <= interval$end.date]
  
  # Find the patients at risk, those whose first visit is on or before the year-end
  first_visits <- outpatient.visit.dt[, .(first_visit = min(VisitDate)), by = PatientID]
  at_risk <- first_visits[first_visit <= interval$end.date, PatientID]
  
  # Get the alive records, patients whose death date is during or after the start of the year
  alive.dt <- mortality.dt[DateOfDeath >= interval$start.date | is.na(DateOfDeath), PatientID]
  
  # Calculate the number of deaths for patients who are at risk
  n_deaths <- nrow(deaths.dt)
  
  # Calculate the number of patients at risk
  n_at_risk <- length(intersect(at_risk, alive.dt))
  
  # Calculate the mortality rate as the percentage of deaths relative to patients at risk
  mortality_rate <- (n_deaths / n_at_risk) * 100
  
  return(mortality_rate)
}

# Example usage: Calculate mortality rate for 2013
get_mortality_rate(2013)
``` 

```{r}
# Test that the rate for 2013 is correct
test_that("2013 is correct", { 
  expect_equal(get_mortality_rate(2013), 1.638896) 
})
```
